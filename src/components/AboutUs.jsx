...
export default function AboutUs() {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting && !hasAnimatedRef.current) {
            ...
            // Subtle parallax effect on scroll (continuous)
            scrollHandler = () => {
              ...
              gsap.to(imageRef.current, {
                scale: 1 + (scrollProgress * (isMobile ? 0.02 : 0.05)),
                rotation: scrollProgress * (isMobile ? 1 : 2),
                duration: 0.3,
                ease: 'none'
              });
            };
  ...
  return (
    <section ref={sectionRef} className="bg-gray-50 w-full overflow-x-hidden py-12 sm:py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row items-center gap-6 sm:gap-8 lg:gap-12">
          ...
          <div ref={imageRef} className="w-full md:w-1/2 h-64 sm:h-72 md:h-96 lg:h-[28rem]">
            <LiquidGlassImage
              src={heRo2}
          ...
          <div ref={contentRef} className="w-full md:w-1/2">
            <h1 ref={titleRef} className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-800 mb-4 sm:mb-6 relative">
              About New Relief International
              <span className="absolute bottom-0 left-0 w-12 sm:w-16 h-1 bg-amber-500 -mb-2"></span>
            </h1>
            
            <p 
              ref={el => paragraphsRef.current[0] = el}
              className="text-base sm:text-lg text-gray-700 mb-3 sm:mb-4 leading-relaxed"
            >
            ...
            <a 
              ...
              href="#" 
              className="inline-block px-5 sm:px-6 py-2.5 sm:py-3 bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white font-medium rounded transition-colors duration-300 ease-in-out text-sm sm:text-base touch-manipulation"
            >
              Learn More About Us
            </a>
          </div>
